...
UKNWN_8 <- aggregate(sub.area~sample+uid+sample.area+super.area+pfs,UKNWN_8,sum)
PercParental <- UKNWN_8$sub.area/UKNWN_8$sample.area*100
...
jPalette<-setNames( RColorBrewer::brewer.pal(9,'Paired'),sort(unique(bloodyMix$pfs)))
  ...
  postscript(file = file.path("D:/Luigi/ALL_POSSIBLE_DAICHI_4/POST_SEGMENTATION_REVAMP_003-RERUN3/EXTRA",
                              "UKNWN_8_percentage of total_vs_CD33MONO.eps"),
             onefile = F,
             width = 7.5,
             height = 7.5,
             horizontal = F,
