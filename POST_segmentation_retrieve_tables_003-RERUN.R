...
analFolder_previous<-"D:/Luigi/ALL_POSSIBLE_DAICHI_4/POST_SEGMENTATION_REVAMP_003"
analFolder<- "D:/Luigi/ALL_POSSIBLE_DAICHI_4/POST_SEGMENTATION_REVAMP_003-RERUN3"

dir.create(analFolder)
...
expData<-st_read(file.path(analFolder_previous,'bigTables','fullSet_biexp_trans.sqlite'))
colnames(expData)[1]<-'pIDs'
colnames(expData)[40]<-'geom'
st_geometry(expData)<-'geom'
#######

#### de-coment this if starting from beginning
# rm(expTable)
...
postscript(file = file.path(analFolder,'Umap','uMap_Reteined_ASP1.eps'),
           onefile = F,
           width = 7.5,
           height = 7.5,
           horizontal = F,
...
postscript(file = file.path(analFolder,'Umap','uMap_ReJected_ASP1.eps'),
           onefile = F,
           width = 7.5,
           height = 7.5,
           horizontal = F,
...
importanceTable<-read.csv(file.path(analFolder_previous,'bigTables','importanceTable.csv'),
                          header = T,
                          row.names = 1,
                          check.names = F)
...
ListbbBox<-matrix(c(8,10,-8.5,-7,
                    8,12,-7,-2,
                    11.25,14.75,-3,0.5,
                    -10,10,3,13,
                    2,7,-8,0,
                    -7,-1,-12,-8,
                    -10,-1,-5,1),ncol=4,byrow = T)
...
for (i in 1:length(mrkr)){
  ...
  postscript(file = file.path(analFolder,'subClusters',paste0('Umap_',mrkr[i],'_ASP1.eps')),
             onefile = F,
             width = 7.5,
             height = 7.5,
             horizontal = F,
  ...
  plot(expGate[expGate$primary_id==mrkr[i] & !expGate$dump,c('uMAP1','uMAP2')],
       ...
       xlab='uMAP1',ylab='uMAP2',asp=1,
       xlim=ListbbBox[i,1:2],ylim=ListbbBox[i,3:4])
...
jPalette<-setNames( RColorBrewer::brewer.pal(9,'Paired'),sort(unique(av_DF$pfs)))
...
for (i in unique(av_DF$class)){
  ...
  postscript(file = file.path(analFolder,"SUPER.POP_pfs",paste0(i,'.eps')),
             ...
             bg='white')
  par(xpd=T)
  plot(data.frame(jitter(df_box$pfs,3),
                  df_box$`% of total area`),
       bg=jPalette[as.character(df_box$pfs)],
       col='gray40',
       pch=21,
  ...
  for (ii in 1:nrow(avrsdr)){
    ...
    lines(matrix(c(avrsdr$pfs[ii],avrsdr$pfs[ii],avrsdr$`mean+sd`[ii],avrsdr$`mean-sd`[ii]),
                 ncol = 2,
                 byrow = F),
          col=jPalette[as.character(avrsdr$pfs[ii])],
          lwd=2)
...
grAnnotationC<-data.frame(`Donor type`=factor(prettyLabels[rn],levels=prettyLabels[colnames(TEMP.sub)],ordered = T),
                          ...
                          row.names = rownames(TEMP.big),
                          check.rows = F)
grAnnotation_color<-list(`Donor type`=grPalette[levels(grAnnotationC$`Donor type`)],
                         `Acceptor type`=grPalette[levels(grAnnotationR$`Acceptor type`)])
...
grAnnotationR<-data.frame(`Acceptor types`=factor(prettyLabels[rn.super],levels=prettyLabels[levels(expGate$primary_id)],ordered = T),
                          ...
                          row.names = rownames(TEMP.big),
                          check.rows = F)
grAnnotation_color<-list(`Donor types`=grPalette.super[levels(grAnnotationC$`Donor types`)],
                         `Donor Clusters` = grPalette.sub[levels(grAnnotationC$`Donor Clusters`)],
                         `Acceptor types`=grPalette.super[levels(grAnnotationR$`Acceptor types`)],
                         `Acceptor Clusters` = grPalette.sub[levels(grAnnotationR$`Acceptor Clusters`)])
